module physpkg
  ...
  logical :: simple_radiation = .true.  ! Flag to try to run radiation -> should be set by user (BPM)
...
!=======================================================================
  ...
  subroutine phys_register
    ...
    use radiation,          only: radiation_register ! BPM
  ...
  subroutine phys_init( phys_state, phys_tend, pbuf2d, cam_in, cam_out )
    ...
    use radiation,           only: radiation_init  ! BPM
...
!$OMP PARALLEL DO PRIVATE (C, NCOL, phys_buffer_chunk)
  ...
  subroutine tphysbc (ztodt, state, tend, pbuf, cam_out, cam_in )
    ...
    use radiation,         only: radiation_tend  ! BPM
    use dycore,            only: dycore_is
    ...
    !===================================================
    call radiation_tend(state, ptend, pbuf, cam_in)
    ! note: CAM has cam_out and cam_in as arguments to radiation_tend
    !       - cam_in provides surface albedos to CAM/RRTMG(P)
    !       - cam_out provides the surface fluxes to the surface models (direct/diffuse)
    call physics_update(state, ptend, ztodt, tend)
    !== End Radiation ==================================
